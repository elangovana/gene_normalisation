...
class TopKCrossEntropyLoss(nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        top_k_loss = torch.mean(torch.topk(non_zero_average_loss_per_item, k=k)[0])
        self._logger.debug(
            "Total loss {} vs topk loss {}".format(torch.mean(non_zero_average_loss_per_item), top_k_loss))
